...
using static SlotSixiangView;

namespace Games.Slot_Games
{
    public class BaseSlotXisiang : BaseGameView
    {
        public static BaseSlotXisiang Instance = null;
        ...
        protected List<Payline> spinPayLines = new List<Payline>();
        ...
        public virtual async UniTask HandleGetInfo(SlotDesk data)
        {
            ...
            // GET_INFO_DATA = data;
            betLevels = data.BetLevels.Select(x => (int)x).ToList();
            agPlayer = data.GameReward.BalanceChipsWalletAfter;
            lbUserAmount.Text = Utility.FormatNumber(agPlayer);
            jackpotLevel = new List<int>
                {(int)data.WinJpHistory.Minor.Ratio, (int)data.WinJpHistory.Major.Ratio, 
                    (int)data.WinJpHistory.Mega.Ratio, (int)data.WinJpHistory.Grand.Ratio };
            // payTalbe = data["payTable"].ToObject<List<List<int>>>();
            currentBetLevel = betLevels.IndexOf((int) data.ChipsMcb);
            setBetLevel((int) data.ChipsMcb);
        }
        ...
        protected void onClickChangeBetLevel(string type)
        {
            ...
            {
                if (isChangeBet || gameState == GAME_STATE.SPINNING)
                {
                    return;
                }
                ...
                lbBetLevel.text = Utility.FormatNumber(validBetLevels[currentBetLevel]);
        ...
        public virtual async UniTask allCollumStopCompleted()
        {
        ...
        {
            if (nodeAutoSpin.activeSelf == false && gameType == (int)SiXiangGame.Normal)
            {
        ...
        public async UniTask handleStopSpin()
        {
        ...
        public virtual void HandleNormalSpin(SlotDesk data)
        {
            ...
            // spinData = data;
            // 1. Xây dựng lại matrix theo cột
            var perChunk = 3;
            var matrix = new List<SiXiangSymbol>();
            int cols = data.Matrix.Cols;
            var flatList = data.Matrix.Lists;
            ...
            // 2. Tạo reels bằng cách chunk array theo perChunk
            var symbolIndices = matrix.Select(v => SymbolDictionary[v]).ToList();
            
            for (int i = 0; i < symbolIndices.Count; i++)
            {
                int chunkIndex = i / perChunk;
                if (spinReelView.Count <= chunkIndex)
                {
                    spinReelView.Add(new List<int>());
                }
                spinReelView[chunkIndex].Add(symbolIndices[i]);
            }

            Debug.Log("ff " +spinReelView.ToString());
            // spinReelView = data["reels"].ToObject<List<List<int>>>();
            spinPayLines =  data.Paylines.ToList();
            agPlayer = (int)data.GameReward.BalanceChipsWalletAfter;
            setNearSpinEffect();
            handleStopSpin();
        }
        ...
        public void setWinType(long winAmount)
        {
            ...
            if (isGrandJackpot)
            {
                winAmount += validBetLevels[currentBetLevel] * jackpotLevel[3];
            }
        ...
        protected async UniTask showWinLine()
        {
            if (spinPayLines.Count > 0)
            {
                ...
                {
                    Payline dataLine = spinPayLines[i];
                    int[] payLineWin = PaylineIdList[dataLine.Id-1];
                    ColorUtility.TryParseHtmlString(listColor[dataLine.Id % listColor.Count], out Color colorLine);
                    listPos.Clear();
                    for (int j = 0; j < 5; j++)
                    {
                        listPos.Add(
                            lineContainer.transform.InverseTransformPoint(listCollum[j]
                                .getPosSymbol(payLineWin[j] + 1)));
                    }
            ...
            if (autoSpinRemain == 0)
            {
                ...
                for (int i = 0, l = spinPayLines.Count; i < l; i++)
                {
                    ...
                    UnityMainThreadDispatcher.Instance.Enqueue(() =>
                    {
                        ...
                        hideAllSymbol();
                        listPos.Clear();
                        Payline dataLine = spinPayLines[index];
                        int[] payLineWin = PaylineIdList[dataLine.Id-1];
                        int numberOfSymbols = dataLine.NumOccur;
                        ColorUtility.TryParseHtmlString(listColor[dataLine.Id % listColor.Count], out Color colorLine);
                        infoBar.setInfoWinLine(SymbolDictionary[dataLine.Symbol], numberOfSymbols,
                            (int) dataLine.Chips);
                ...
                UnityMainThreadDispatcher.Instance.Enqueue(() => activeAllSymbol());
            }
        }
        ...
        protected void setBetLevel(int betInit = 0)
        {
            ...
            {
                lbBetLevel.text = Utility.FormatNumber(betInit);
            }
            else
            {
                lbBetLevel.text = Utility.FormatNumber(validBetLevels[currentBetLevel]);
        ...
        protected void drawLines(List<Vector2> listPos, Color colorLine)
        {
            ...
            Vector2 lastPos = new Vector2(listPos[^1].x + 80, listPos[^1].y);
        ...
        public async UniTask showResultMoneyAnim(string path, string animName, long chipWin, Vector2 posLbMoney)
        {
            ...
            Action<SkeletonDataAsset> cb = async (skeData) =>
            {
                ...
                await UniTask.Delay(
                    TimeSpan.FromSeconds(spineJackpotWin.Skeleton.Data.FindAnimation(animName).Duration + 1.0f));
        ...
        protected UniTask showSpineSpecialWin(WIN_TYPE wintype, long chipWin = 0)
        {
            ...
            if (wintype != WIN_TYPE.NORMAL)
            {
                ...
                {
                    string animName = "";
                    spineSpecialWin.skeletonDataAsset =
                        skeData; // UIManager.instance.loadSkeletonData(PATH_ANIM_SPECICAL_WIN);
                    ...
                    spineSpecialWin.AnimationState.Complete += delegate { hideSpineSpecical(); };
                };
                UnityMainThreadDispatcher.Instance.Enqueue(() =>
                {
        ...
        public async UniTask showSpineJackpotWin(long chipWin)
        {
            ...
            {
                string animName = "";
                spineJackpotWin.skeletonDataAsset =
                    skeData; // UIManager.instance.loadSkeletonData("GameView/SiXiang/Spine/LuckyDraw/BigWin/skeleton_SkeletonData");
                // spineBgMoney.skeletonDataAsset = UIManager.instance.loadSkeletonData(PATH_ANIM_SPECICAL_WIN);
                await UniTask.Delay(TimeSpan.FromSeconds(0.1f));
        ...
        public void onClickSkipSpecialSpine()
        {
            ...
            {
                float duration = spineSpecialWin.Skeleton.Data.FindAnimation(spineSpecialWinTrack.Animation.Name)
                    .Duration;
