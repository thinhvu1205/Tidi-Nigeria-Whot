...
public class FriendSendGiftItem : MonoBehaviour
{
    ...
    public void SetInfo(FriendGiftItem item)
    {
        ...
        imageBackground.color = item.VipUnlock <= User.Profile.Vip ? Color.white : Color.gray;
        imageGift.color = item.VipUnlock <= User.Profile.Vip ? Color.white : Color.gray;
        textPrice.color = item.VipUnlock <= User.Profile.Vip ? Color.white : Color.gray;
    }
}
